...
enum vk_poll_method {
	VK_POLL_METHOD_LEVEL_TRIGGERED_ONESHOT =
	    0, /* register whenever there is a block (poll() and kqueue() get this for free due to registration
		  batching, but epoll() incurs extra system calls) */
	VK_POLL_METHOD_EDGE_TRIGGERED =
...
int vk_fd_table_using_io_uring(struct vk_fd_table* fd_table_ptr);
