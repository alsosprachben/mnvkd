...
int vk_vectoring_vector_within(const struct vk_vectoring* ring, const struct iovec* vector)
{
	return ((char*)vector->iov_base) > ring->buf_start &&
	       ((char*)vector->iov_base) + vector->iov_len <= ring->buf_start + ring->buf_len;
}
...
{
	if (!vk_vectoring_vectors_within(ring, ring->vector_rx) ||
	    !vk_vectoring_vectors_within(ring, ring->vector_tx)) {
		ring->error = EINVAL;
	}
}

size_t vk_vectoring_tx_cursor(const struct vk_vectoring* ring) { return ring->tx_cursor; }
size_t vk_vectoring_tx_len(const struct vk_vectoring* ring) { return ring->tx_len; }
size_t vk_vectoring_rx_cursor(const struct vk_vectoring* ring)
{
	return (ring->tx_cursor + ring->tx_len) % ring->buf_len;
}
size_t vk_vectoring_rx_len(const struct vk_vectoring* ring) { return ring->buf_len - ring->tx_len; }

int vk_vectoring_printf(const struct vk_vectoring* ring, const char* label)
{
	return dprintf(2,
		       "%s: rx(%zu+%zu/%zu) tx(%zu+%zu/%zu) rx[0].iov_len = %zu, rx[1].iov_len = %zu, tx[0].iov_len = "
		       "%zu, tx[1].iov_len = %zu, rx buffer = \"%.*s%.*s\", tx buffer = \"%.*s%.*s\"\n",
		       label, vk_vectoring_rx_cursor(ring), vk_vectoring_rx_len(ring), vk_vectoring_buf_len(ring),
		       vk_vectoring_tx_cursor(ring), vk_vectoring_tx_len(ring), vk_vectoring_buf_len(ring),
		       ring->vector_rx[0].iov_len, ring->vector_rx[1].iov_len, ring->vector_tx[0].iov_len,
		       ring->vector_tx[1].iov_len, (int)ring->vector_rx[0].iov_len, (char*)ring->vector_rx[0].iov_base,
...
int vk_vectoring_rx_buf1_len(const struct vk_vectoring* ring) { return (int)ring->vector_rx[0].iov_len; }
int vk_vectoring_rx_buf2_len(const struct vk_vectoring* ring) { return (int)ring->vector_rx[1].iov_len; }
int vk_vectoring_tx_buf1_len(const struct vk_vectoring* ring) { return (int)ring->vector_tx[0].iov_len; }
int vk_vectoring_tx_buf2_len(const struct vk_vectoring* ring) { return (int)ring->vector_tx[1].iov_len; }
char* vk_vectoring_rx_buf1(const struct vk_vectoring* ring) { return (char*)ring->vector_rx[0].iov_base; }
char* vk_vectoring_rx_buf2(const struct vk_vectoring* ring) { return (char*)ring->vector_rx[1].iov_base; }
char* vk_vectoring_tx_buf1(const struct vk_vectoring* ring) { return (char*)ring->vector_tx[0].iov_base; }
char* vk_vectoring_tx_buf2(const struct vk_vectoring* ring) { return (char*)ring->vector_tx[1].iov_base; }
...
/* has error */
int vk_vectoring_has_error(struct vk_vectoring* ring) { return ring->error; }

/* has EOF */
int vk_vectoring_has_eof(struct vk_vectoring* ring) { return ring->eof; }

/* no bytes in transmit buffer */
int vk_vectoring_is_empty(struct vk_vectoring* ring) { return vk_vectoring_vector_tx_len(ring) == 0; }

/* has EOF and has no data */
int vk_vectoring_has_nodata(struct vk_vectoring* ring) { return ring->eof && vk_vectoring_is_empty(ring); }

/* coroutines should be affected */
int vk_vectoring_has_effect(struct vk_vectoring* ring) { return ring->effect; }
...
/* splice data from vector-ring to vector-ring -- this is the internal read/write op -- if len is < 0, splice all
 * available bytes */
