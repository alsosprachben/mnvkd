...
##### Responder Pipeline with `vk_join()`

Parent streams messages to a responder coroutine, then joins it to ensure the
pipeline drains before closing the pipe.

From [`vk_test_responder_join.c`](vk_test_responder_join.c):
```c
struct join_message {
	int close;
	size_t len;
...
void parent(struct vk_thread *that)
{
	struct {
