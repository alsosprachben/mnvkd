...
struct vk_pipe* vk_socket_get_reader_pipe(struct vk_socket* socket_ptr) { return &socket_ptr->rx_fd; }
struct vk_pipe* vk_socket_get_writer_pipe(struct vk_socket* socket_ptr) { return &socket_ptr->tx_fd; }
...
void vk_socket_mark_read_block(struct vk_socket* socket_ptr) { socket_ptr->block.ioft_rx_pre.event.events = POLLIN; }
...
void vk_socket_handle_block(struct vk_socket* socket_ptr)
{
	...
	if (vk_pipe_get_fd(&socket_ptr->rx_fd) >= 0 && vk_pipe_get_closed(&socket_ptr->rx_fd)) {
		...
		vk_io_future_set_rx_closed(vk_block_get_ioft_rx_pre(vk_socket_get_block(socket_ptr)),
					   vk_pipe_get_closed(&socket_ptr->rx_fd));
	}
	/* for write */
	...
	if (vk_pipe_get_fd(&socket_ptr->tx_fd) >= 0 && vk_pipe_get_closed(&socket_ptr->tx_fd)) {
		/* Forward closed state from pipe to IO future. */
		vk_io_future_set_tx_closed(vk_block_get_ioft_tx_pre(vk_socket_get_block(socket_ptr)),
					   vk_pipe_get_closed(&socket_ptr->tx_fd));
	}
		...
		case VK_PIPE_OS_FD:
			switch (socket_ptr->rx_fd.fd_type) {
				case VK_FD_TYPE_SOCKET_LISTEN:
					rc = vk_vectoring_accept(&socket_ptr->rx.ring,
		...
		case VK_PIPE_OS_FD:
			...
			switch (socket_ptr->tx_fd.type) {
				case VK_PIPE_OS_FD:
					...
					rc = vk_vectoring_write(&socket_ptr->tx.ring,
				...
				case VK_PIPE_VK_RX:
					...
					rc = vk_vectoring_splice(vk_pipe_get_rx(&socket_ptr->tx_fd),
								 &socket_ptr->tx.ring, -1);
					if (rc == -1) {
						vk_socket_handle_error(socket_ptr);
					}
					rc = 0;
...
int vk_socket_get_enqueued_blocked(struct vk_socket* socket_ptr) { return socket_ptr->blocked_enq; }
...
struct vk_pipe* vk_socket_get_rx_fd(struct vk_socket* socket_ptr) { return &socket_ptr->rx_fd; }

struct vk_pipe* vk_socket_get_tx_fd(struct vk_socket* socket_ptr) { return &socket_ptr->tx_fd; }

struct vk_vectoring* vk_socket_get_rx_vectoring(struct vk_socket* socket_ptr) { return &socket_ptr->rx.ring; }

struct vk_vectoring* vk_socket_get_tx_vectoring(struct vk_socket* socket_ptr) { return &socket_ptr->tx.ring; }

struct vk_block* vk_socket_get_block(struct vk_socket* socket_ptr) { return &socket_ptr->block; }

/* check EOF flag on socket -- more bytes may still be available to receive from socket */
int vk_socket_eof(struct vk_socket* socket_ptr) { return vk_vectoring_has_eof(vk_socket_get_rx_vectoring(socket_ptr)); }
...
void vk_socket_clear(struct vk_socket* socket_ptr) { vk_vectoring_clear_eof(vk_socket_get_rx_vectoring(socket_ptr)); }
...
int vk_socket_get_error(struct vk_socket* socket_ptr) { return socket_ptr->error; }
...
int vk_block_get_op(struct vk_block* block_ptr) { return block_ptr->op; }
...
size_t vk_block_get_committed(struct vk_block* block_ptr) { return block_ptr->copied; }

size_t vk_block_get_uncommitted(struct vk_block* block_ptr) { return block_ptr->len; }

void vk_block_set_uncommitted(struct vk_block* block_ptr, size_t len) { block_ptr->len = len; }

char* vk_block_get_buf(struct vk_block* block_ptr) { return block_ptr->buf; }

void vk_block_set_buf(struct vk_block* block_ptr, char* buf) { block_ptr->buf = buf; }

size_t vk_block_get_len(struct vk_block* block_ptr) { return block_ptr->len; }

void vk_block_set_len(struct vk_block* block_ptr, size_t len) { block_ptr->len = len; }

struct vk_io_future* vk_block_get_ioft_rx_pre(struct vk_block* block_ptr) { return &block_ptr->ioft_rx_pre; }
struct vk_io_future* vk_block_get_ioft_tx_pre(struct vk_block* block_ptr) { return &block_ptr->ioft_tx_pre; }
struct vk_io_future* vk_block_get_ioft_rx_ret(struct vk_block* block_ptr) { return &block_ptr->ioft_rx_ret; }
struct vk_io_future* vk_block_get_ioft_tx_ret(struct vk_block* block_ptr) { return &block_ptr->ioft_tx_ret; }

struct vk_thread* vk_block_get_vk(struct vk_block* block_ptr) { return block_ptr->blocked_vk; }

void vk_block_set_vk(struct vk_block* block_ptr, struct vk_thread* blocked_vk) { block_ptr->blocked_vk = blocked_vk; }
