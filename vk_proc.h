...
int vk_proc_get_deferred(struct vk_proc* proc_ptr);
void vk_proc_set_deferred(struct vk_proc* proc_ptr, int deferred);
...
/* next proc in the kernel deferred queue */
...
#define PRproc "<proc       id=\"%4zu\" run=\"%c\" blocked=\"%c\" deferred=\"%c\">"
#define ARGproc(proc_ptr)                                                                                              \
	vk_proc_get_id(proc_ptr),                                                                                        \
	    (vk_proc_get_run(proc_ptr) ? 't' : 'f'),                                                                     \
	    (vk_proc_get_blocked(proc_ptr) ? 't' : 'f'),                                                                 \
	    (vk_proc_get_deferred(proc_ptr) ? 't' : 'f')
