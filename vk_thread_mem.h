...
#define vk_calloc(val_ptr, nmemb) \
	(val_ptr) = vk_stack_push(vk_heap_get_stack(vk_proc_get_heap(vk_get_proc(that))), (nmemb), sizeof (*(val_ptr))); \
	if ((val_ptr) == NULL) { \
		vk_error(); \
	}

#define vk_calloc_size(val_ptr, size, nmemb) \
	(val_ptr) = vk_stack_push(vk_heap_get_stack(vk_proc_get_heap(vk_get_proc(that))), (nmemb), size); \
	if ((val_ptr) == NULL) { \
		vk_error(); \
	}
...
#define vk_free()                                          \
	rc = vk_stack_pop(vk_heap_get_stack(vk_proc_get_heap(vk_get_proc(that)))); \
	if (rc == -1) {                                        \
