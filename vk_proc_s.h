...
struct vk_proc {
	...
	SLIST_ENTRY(vk_proc) deferred_list_elem; /* kern-rw; head on `struct vk_kern` member `deferred_procs` */
	...
	int privileged;	     /* whether to not protect the kernel when running, for processes that need to spawn other
				processes */
	int isolated;	     /* whether to protect the process when not running */
};
