...
void vk_signal_init() {
    ...
    vk_signal_register(&global_vk_signal);
}

void vk_signal_setjmp() {
    int rc;

    rc = sigsetjmp(global_vk_signal.env, 1);
    if (rc == 1) {
...
void vk_signal_handler(struct vk_signal *signal_ptr, int signal, siginfo_t *siginfo_ptr, ucontext_t *uc_ptr) {
    ...
    if (jump) {
        ...
        siglongjmp(signal_ptr->env, 1);
    }
}

void vk_signal_register(struct vk_signal *signal_ptr) {
    int rc;
    ...
    /* design logic */
    rc = sigaction(SIGPIPE, SIG_IGN, 0);
    if (rc == -1) {
        return -1;
    }
