...
struct vk_fd {
	...
	size_t proc_id; /* logical process ID (only one physical process: `getpid()`) -- the `struct vk_proc` and
			   `struct vk_proc_local` it is attached to */
	/* There is a 1:1 relationship between `fd` and `proc_id`.
	 * Therefore, an FD can only be attached to a single logical process.
	 * Use `socketpair()` or `pipe2()` to communicate between protected logical or physical processes on the same
	 * system.
	 */
	...
	SLIST_ENTRY(vk_fd)
	allocated_list_elem;		    /* element in tracked list -- head on `struct vk_proc` at `allocated_fds` */
	...
	/* registration lifecycle --  pre and ret, the logical state, is also represented in `struct vk_block` in
	 * `struct vk_socket` */
	struct vk_io_future ioft_post; /* state registered or polled: physical, posterior -- in poller */
	struct vk_io_future
	    ioft_pre; /* state to register:          logical,  prior     -- also via `struct vk_block` */
	struct vk_io_future
	    ioft_ret; /* state to dispatch:          logical,  posterior -- also via `struct vk_block` */
