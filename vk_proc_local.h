...
/* set supervisor CR, or NULL to disable -- if a supervisor is set, signals are delivered to it instead of the running
 * thread */
...
#define ARGprocl(proc_local_ptr)                                                                                       \
	vk_proc_local_get_proc_id(proc_local_ptr), (vk_proc_local_get_run(proc_local_ptr) ? 't' : 'f'),                \
	    (vk_proc_local_get_blocked(proc_local_ptr) ? 't' : 'f')

#define vk_proc_local_logf(fmt, ...)                                                                                   \
	ERR("proc_local: " PRloc " " PRprocl ": " fmt, ARGloc, ARGprocl(proc_local_ptr), __VA_ARGS__)
#define vk_proc_local_dbgf(fmt, ...)                                                                                   \
	DBG("proc_local: " PRloc " " PRprocl ": " fmt, ARGloc, ARGprocl(proc_local_ptr), __VA_ARGS__)
#define vk_proc_local_log(note)                                                                                        \
	ERR("proc_local: " PRloc " " PRprocl ": "                                                                      \
	    "%s\n",                                                                                                    \
	    ARGloc, ARGprocl(proc_local_ptr), note)
#define vk_proc_local_dbg(note)                                                                                        \
	DBG("proc_local: " PRloc " " PRprocl ": "                                                                      \
	    "%s\n",                                                                                                    \
	    ARGloc, ARGprocl(proc_local_ptr), note)
