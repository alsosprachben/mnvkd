...
int vk_proc_execute(struct vk_proc *proc_ptr) {
    ...
    if (proc_ptr->local_ptr->running_cr) {
    ...
    while ( (that = vk_proc_dequeue_run(proc_ptr)) ) {
        ...
        while (vk_is_ready(that)) {
            ...
            proc_ptr->local_ptr->running_cr = that;
            func(that);
            proc_ptr->local_ptr->running_cr = NULL;
            DBG("  STOP@"PRIvk"\n", ARGvk(that));
        ...
        SLIST_FOREACH(that, &proc_ptr->local_ptr->run_q, run_q_elem) {
            DBG("   INQ@"PRIvk"\n", ARGvk(that));
        }
	}
